  ...
  let invoiceItems = itemsList.map((item, index)=>(
    <tr key={index}>
      <InvoiceItem 
        quantity={item.quantity}
        id={item.id}
        price={(Math.round((item.price - (item.price*discountPercentage/100)) * 10000 * rate / (vatPercentage+100)) / 100).toFixed(2)}
        total={(Math.round((item.totalPrice - (item.totalPrice*discountPercentage/100)) * 10000 * rate / (vatPercentage+100)) / 100).toFixed(2)}
        name={item.name}
        product={item.product}/>
    </tr>
  ))
        ...
        <table className="w-100">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style={{fontSize: '14px', textAlign:'right'}} className='border-top-xl border-bottom-xl'>${(Math.round(subTotal * 100 * rate) / 100).toFixed(2)}</td>
            </tr> 
            {
              clientLevel===2?
              <tr>
            ...
            <tr>
              ...
              <td style={{fontSize: '14px', textAlign:'right'}} className='border-top-xl border-bottom-xl'>${total}</td>
            </tr>
          </tbody>
        </table>
