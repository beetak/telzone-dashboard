...
import { fetchAsyncSalesByCurrencyId, fetchAsyncSalesByRegion, fetchAsyncSalesByShop, fetchAsyncSalesByTown, getAgentLoadingStatus, getAgentSales, getAllSales, getLoadingByCurIdStatus, getRegionSales, getShopSales, getTotalSales, getTownSales, saleActions } from '../../store/sales-slice';
...
export default function SummaryTaxes() {
    ...
    if(count>0){
        ...
        agentSalesData = 
        <tr>
        <td colspan={7} className='text-center'><h5 style={{color: '#0C55AA'}}>No {currencyState ==='Currency'?'':currencyState ==='USD'?'USD':currencyState ==='ZiG'&&'ZiG'} Taxes Found</h5></td>
        </tr>
    }
    ...
    let errorMsg1 =  
        ...
        if(userRole === 'Regional Accountant'){
            selectionLevel = <>
                {/* Town Dropdown */}
                ...
                <div className="card h-100" ref={componentRef} style={{width: '100%', height: window.innerHeight, padding: '80px'}}>
                    ...
                    <div className="card-body p-3 pb-0">
                        <table className="table align-items-center mb-0 p-5">
                            ...
                            <tbody>
                                {
                                    userRole === "Super Admin" || userRole === "Admin" || userRole === "Head Retail" || userRole === "Finance Manager" ?
                                    (
                                        <>
                                            <tr>Online Sales</tr>
                                            {
