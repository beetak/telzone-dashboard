      ...
      <td>
        <div className="d-flex px-2 py-1">
          <div className="d-flex flex-column justify-content-center">
            <h6 className="mb-0 text-sm">{data.mac}</h6>
          </div>
        </div>
      </td>
      <td>
        <p className="text-xs font-weight-bold mb-0">{data.os}</p>
      </td>
      <td>
        <p className="text-xs font-weight-bold mb-0">{data.status}</p>
      </td>
      <td>
        <p className="text-xs font-weight-bold mb-0">{data.recentDeviceConnection}</p>
      </td>
      {/*<td className="align-middle">
        <a className="btn btn-link text-dark px-3 mb-0" onClick={() => {
            dispatch(fetchAsyncUsageReports(data.mac)); openUpdateModal()
          }}><i className="material-icons text-sm me-2">edit</i>View Usage
        </a>
        </td>*/}
